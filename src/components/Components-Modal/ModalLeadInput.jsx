	...
	return (
		<div className="p-[10px] mt-[20px] text-[18px] h-[500px]">
			...
			<div className=" flex flex-wrap items-center bg-white border mt-[10px] rounded-lg shadow-sm  p-[10px] ">
				{leadSrc.length
					? leadSrc.map((item, index) => (
							<div
								key={index}
								className=" border mr-1  mb-1 text-xs bg-[#e6e6e6] p-1 flex "
							>
								{item.title}
								<RxCross2
									onClick={() => handleDelete(index)}
									className=" ml-1.5 my-auto"
									sx={{ fontSize: "14px" }}
								/>
							</div>
					  ))
				...
				<input
					...
					className={`${placeholder ? "w-[95%]" : ""} focus:outline-none  `}
					onClick={() => {
			...
			{leadSrc.length ? (
				<div className="flex">
					<button
						...
						onClick={() => {
							addNodes("leadSourceNode", leadSrc, "Leads from");
							toggleModal(false, {});
							setLeadSrc([]);
							toggleSelect(false);
						}}
