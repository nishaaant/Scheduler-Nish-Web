...
const ModalButton = () => {
	...
	const data = {
		title:
			modalData.type === "addLeadSource" ? "Leads from List(s)" : "Cold Email",
		subTitle:
	...
	return (
		<div className="p-[10px] h-[500px] cursor-pointer">
			...
			<div
				className="mt-[10px] bg-[#f9f9f9]  w-[400px] p-[15px] rounded-[5px] shadow-lg  border  border-[#d7d7d7] flex items-center gap-4"
				onClick={() => {
			...
			>
				<div
					className={`h-[60px] w-[60px] p-[10px] border ${
						modalData.type === "addLeadSource"
							? " border-[#f484b2] bg-[#fbd6e5]"
							: " border-[#9f52e6] bg-[#e3ccf8]"
					} rounded-[5px]`}
				>
					{modalData.type === "addLeadSource" ? (
			...
			{addedData === "email" && modalData.type !== "addLeadSource" ? (
				<>
					...
					>
						<div
							className={`h-[60px] w-[60px] p-[10px] border border-[#93ccf3] bg-[#eaf1f7]  rounded-[5px]`}
						>
							<FaRegHourglassHalf
								style={{ color: "#03a6ff", fontSize: "40px" }}
							/>
						</div>
						<div>
